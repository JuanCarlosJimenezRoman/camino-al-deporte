      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto px-2 py-3">
          {secciones.map((seccion) => (
            <div key={seccion.titulo} className="mb-5">
              <div
                className={`px-3 mb-1.5 text-[11px] font-semibold uppercase tracking-wider text-sidebar-foreground/40 ${
                  collapsed ? 'md:hidden' : ''
                }`}
              >
                {seccion.titulo}
        ...
        <div className="border-t border-sidebar-border p-2 hidden md:block">
          <button
            ...
            className="w-full flex items-center gap-3 rounded-lg px-3 h-9 text-sm font-medium text-sidebar-foreground/70 hover:bg-secondary hover:text-sidebar-foreground transition-colors"
          >
