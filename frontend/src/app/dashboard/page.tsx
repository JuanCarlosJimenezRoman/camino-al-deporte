...
import { Package, Store, ShoppingCart, Users, AlertTriangle, CalendarClock, ArrowRight } from 'lucide-react';
...
export default function DashboardHome() {
  ...
  useEffect(() => {
    ...
    async function cargar() {
      ...
      if (puedeVer('apartados', rol)) {
        try {
          const apartados = await api<{ estado: string; saldoPendiente: number }[]>('/apartados?estado=ACTIVO');
          const saldo = apartados.reduce((acc, a) => acc + (a.saldoPendiente || 0), 0);
          resultado.push({
            titulo: 'Apartados activos',
            valor: String(apartados.length),
            descripcion: `$${saldo.toLocaleString('es-MX', { minimumFractionDigits: 2 })} por cobrar`,
            icon: CalendarClock,
            href: '/dashboard/apartados',
            tono: saldo > 0 ? 'warning' : 'success',
          });
        } catch {
          /* noop */
        }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-semibold">Hola, {usuario?.nombre?.split(' ')[0]}</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Aquí tienes un vistazo general. Usa el menú para gestionar productos, inventario o ventas
          según tu rol.
        </p>
      </div>
        ...
        <CardContent className="flex flex-wrap gap-2">
          ...
          {puedeVer('apartados', rol) && (
            <Link href="/dashboard/apartados">
              <Button variant="secondary" size="sm">
                Apartados <ArrowRight className="w-3.5 h-3.5" />
              </Button>
            </Link>
          )}
        </CardContent>
